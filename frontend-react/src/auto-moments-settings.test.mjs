...
test('role editor restores and saves target or system-decided automatic moment frequency', () => {
  assert.match(source, /momentFreq:\s*normalizeMomentFrequency\(role\.auto_moments_daily_max\)/);
  assert.match(source, /momentSystemDecides:\s*Number\(role\.auto_moments_daily_min \|\| 0\) === 0/);
  assert.match(source, /auto_moments_daily_min:\s*auto && !systemDecides \? freq : 0/);
