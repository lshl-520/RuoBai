...
export async function getOwnedCharacter(userId, characterId, connection = pool) {
  ...
  const [rows] = await connection.query(
    `
      SELECT id, user_id, char_key, name, tag, persona, avatar, portrait_id, portrait_custom_url, mood, intimacy, speech_style, chat_credential_id, chat_model_id, chat_thinking_level, is_active, is_deleted, created_at
      FROM characters
...
export async function getActiveCharacter(userId, connection = pool) {
  const [rows] = await connection.query(
    `
      SELECT id, user_id, char_key, name, tag, persona, avatar, portrait_id, portrait_custom_url, mood, intimacy, chat_credential_id, chat_model_id, chat_thinking_level, is_active, is_deleted, created_at
      FROM characters
